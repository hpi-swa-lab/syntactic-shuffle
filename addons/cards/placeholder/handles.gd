...
func _forward_canvas_draw_over_viewport(node: Placeholder, viewport_control: Control):
	if node.handles():
		for handle in node.handles():
			viewport_control.draw_texture(handle.icon, handle.rect_in(node).position)

func _forward_canvas_gui_input(node: Placeholder, event: InputEvent, undo_redo):
