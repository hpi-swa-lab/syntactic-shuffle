@tool
#thumb("slide_vertically.png")
extends Card

func _ready() -> void:
	super._ready()
	setup("Slide Vertically", "Slides the entity vertically.", Card.Type.Effect, [
...
func slide(direction: float):
	...
	velocity_y = lerp(velocity_y, direction * max_velocity, min(1.0, _accel * get_process_delta_time()))
